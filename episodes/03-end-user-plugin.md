...
For the X,Y positions, declare a `TH2D` in the header, book it in `InitWithGlobalRootLock`, and fill
it from `hit->getPosition().x` and `hit->getPosition().y` in `ProcessSequential`. Add an
`if (hit->getEnergy() > 0.005)` guard to make the energy-cut version. To switch detectors, change the
`PrefetchT` collection name from `"EcalBarrelHits"` to `"EcalEndcapNHits"`. Rebuild the plugin after
every change with the two `cmake` commands above.
